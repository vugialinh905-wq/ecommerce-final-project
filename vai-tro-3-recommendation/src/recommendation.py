...
class RecommendationEngine:
    ...
    def export_recommendations(self, output_file: str):
        ...
        FIXED: Properly map product_id to product_name using product_map
        ...
        for customer_id, products in self.recommendations.items():
            for rank, product_id in enumerate(products, 1):
                # Ensure product_id exists in product_map
                if product_id not in self.product_map:
                    product_name = 'Unknown'
                    unmapped_count += 1
                else:
                    product_name = self.product_map[product_id]
        ...
        if data:
            ...
            # Validation report
            if unmapped_count > 0:
                print(f"⚠️  Warning: {unmapped_count} recommendations have unknown product names")
            else:
                print(f"✓ All {len(data)} recommendations mapped successfully")
        else:
